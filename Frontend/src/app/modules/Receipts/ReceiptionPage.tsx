...
import { 
  ...
  InvoiceReference, 
  isInvoicePopulated 
} from '../Receipts/componetReceiption/Payment'
import { MenuComponent } from '../../../_metronic/assets/ts/components'
import ReceiptDetailModal from './componetReceiption/ReceiptDetail' 
...
const ReceiptionPage: React.FC = () => {
  ...
  // 🌟 ແກ້ໄຂ: ຍ້າຍ Pagination Variables ມາໄວ້ໃນນີ້ໃຫ້ຖືກຕ້ອງ
                      ...
                      paginatedData.map((rc, index) => {
                        ...
                        return (
                          <tr key={rc._id || index}>
                            ...
                            <td className='text-primary fw-bold'>{inv?.invoiceNumber || 'N/A'}</td>
                            <td>{cust?.name || 'Unknown'}</td>
                            <td>{rc.paymentDate ? new Date(rc.paymentDate).toLocaleDateString('lo-LA') : '-'}</td>
                            <td className='text-end text-muted'>
                              {formatCurrency(rc.invoiceTotalSnapshot || inv?.grandTotal || 0, rc.currency || 'LAK')} {rc.currency}
                            </td>
